  ...
  <script>
    ...
    class Tower {
      ...
      update(dt) {
        ...
        const tgt = inRange.reduce((a,b) => ((a.x - this.x)**2 + (a.y - this.y)**2 < (b.x - this.x)**2 + (b.y - this.y)**2 ? a : b));
      ...
      draw(ctx) {
        ...
        ctx.fillStyle = '#888'; ctx.beginPath(); ctx.arc(0, 0, 8, 0, 2*Math.PI); ctx.fill();
        ctx.fillStyle = '#666'; ctx.fillRect(0, -4, 16, 8);
        ctx.restore();
      }
      fire() {}
    }
    ...
    class BasicTower extends Tower {
      ...
      fire() { bullets.push(new Bullet(this.x, this.y, Math.cos(this.angle)*300, Math.sin(this.angle)*300, 1, 0)); }
    }
    // Sniper Tower
    class SniperTower extends Tower {
      constructor(x,y) { super(x,y,600,0.3); }
      fire() { bullets.push(new Bullet(this.x, this.y, Math.cos(this.angle)*500, Math.sin(this.angle)*500, 3, 0)); }
      draw(ctx) {
        super.draw(ctx);
    ...
    class FrostTower extends Tower {
      ...
      fire() { bullets.push(new Bullet(this.x,this.y,Math.cos(this.angle)*200,Math.sin(this.angle)*200,0.5,0.4)); }
    ...
    class Eyeball {
      constructor() {
        const base = pathPoints[0]; this.x=base.x; this.y=base.y;
        this.radius=r; this.maxHealth=3; this.health=this.maxHealth;
        this.finsAngle=0; this.blinking=false; this.blinkTime=0;
      ...
      update(dt) {
        ...
        if(dist<5) {
          if(this.targetIndex<pathPoints.length-1) this.targetIndex++;
            ...
            deathEffects.push(new DeathEffect(this.x,this.y,0,moveSpeed));
            return false;
          }
        } else { dx/=dist; dy/=dist; this.x+=dx*moveSpeed*dt; this.y+=dy*moveSpeed*dt; }
        this.finsAngle+=dt*20;
        if(!this.blinking){ this.blinkTimer-=dt; if(this.blinkTimer<=0){this.blinking=true;this.blinkTime=0;} }
        else{ this.blinkTime+=dt; if(this.blinkTime>=this.blinkDuration*2){this.blinking=false;this.blinkTimer=2+Math.random()*3;this.blinkTime=0;} }
        return true;
      }
      draw(ctx) {
        // glow
        ...
        let vx=mx-this.x, vy=my-this.y; const dd=Math.hypot(vx,vy)||1; vx/=dd; vy/=dd;
        ...
        // blink
        if(this.blinking){ let p=this.blinkTime/this.blinkDuration; if(p>1)p=2-p; p=Math.max(0,Math.min(1,p)); const cover=this.radius*2*p;
    ...
    // Bullet & Effects
    class Bullet {
      constructor(x,y,vx,vy,dmg,slow){ this.x=x;this.y=y;this.vx=vx;this.vy=vy;this.size=4;this.dmg=dmg;this.slow=slow; }
      update(dt){ this.x+=this.vx*dt; this.y+=this.vy*dt; }
      draw(ctx){ ctx.fillStyle=this.slow? '#0cf':'#ff0'; ctx.beginPath(); ctx.arc(this.x,this.y,this.size,0,2*Math.PI); ctx.fill(); }
    }
    class DeathEffect {
      constructor(x,y,vx,vy){ this.particles=[];
        ...
        for(let i=0;i<50;i++){ const px=x+(Math.random()-0.5)*r*2, py=y+(Math.random()-0.5)*r*2;
      ...
      draw(ctx){ this.particles.forEach(p=>{ const t=1-p.age/p.life; if(p.type==='goo'){ ctx.fillStyle=`rgba(50,255,50,${t})`; ctx.beginPath(); ctx.arc(p.x,p.y,p.size*t,0,2*Math.PI); ctx.fill(); } else { ctx.fillStyle=`rgba(50,255,50,${t})`; ctx.fillRect(p.x,p.y,p.size,p.size); } }); }
    }

    // Spawn & Game Loop
    function spawnOne(){ eyeballs.push(new Eyeball()); }
    function init(){ eyeballs.length=0; towers.length=0; bullets.length=0; deathEffects.length=0; spawnTimer=0; lives=5; score=0; document.getElementById('lives').textContent=lives; document.getElementById('score').textContent=score; lastTime=performance.now(); requestAnimationFrame(loop); }
    function loop(ts){ const dt=(ts-lastTime)/1000; lastTime=ts;
      // background & path
      ctx.fillStyle='#3a5f0b'; ctx.fillRect(0,0,canvas.width,canvas.height);
      ctx.strokeStyle='#b5651d'; ctx.lineWidth=pathWidth; ctx.lineCap='round'; ctx.beginPath(); ctx.moveTo(pathPoints[0].x,pathPoints[0].y); pathPoints.forEach(p=>ctx.lineTo(p.x,p.y)); ctx.stroke();
      // spawn
      ...
      bullets.forEach((b,bi)=>{ b.update(dt); for(let ei=eyeballs.length-1;ei>=0;ei--){ const e=eyeballs[ei]; if(Math.hypot(e.x-b.x,e.y-b.y)<e.radius+b.size){ e.health-=b.dmg; if(b.slow){ /* optional slow effect */ } deathEffects.push(new DeathEffect(e.x,e.y,b.vx,b.vy)); bullets.splice(bi,1); if(e.health<=0){ eyeballs.splice(ei,1); score++; document.getElementById('score').textContent=score; } break; } }});
      deathEffects.forEach((d,di)=>{ if(!d.update(dt)) deathEffects.splice(di,1); });
      // draws
      deathEffects.forEach(d=>d.draw(ctx)); eyeballs.forEach(e=>e.draw(ctx)); towers.forEach(t=>t.draw(ctx)); bullets.forEach(b=>b.draw(ctx));
      requestAnimationFrame(loop);
    }
    // Place towers
    canvas.addEventListener('click', e=>{ const rect=canvas.getBoundingClientRect(); const x=e.clientX-rect.left, y=e.clientY-rect.top; if(selectedTower==='basic') towers.push(new BasicTower(x,y)); else if(selectedTower==='sniper') towers.push(new SniperTower(x,y)); else if(selectedTower==='frost') towers.push(new FrostTower(x,y)); });
    // Start game
