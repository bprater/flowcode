...
<head>
    ...
    <title>Full Font Control 3D Card</title>
    <style>
        ...
        .panel { position: fixed; top: 10px; width: 330px; /* Slightly wider */ max-height: calc(100vh - 20px);
        ...
        .control-row label { min-width: 120px; /* Increased min-width */ text-align: right; color: #ccc; font-size:0.95em; }
...
<body>
    ...
    <script>
        ...
        const availableFontFamilies = [
            { name: "Arial", value: "Arial, sans-serif" }, { name: "Verdana", value: "Verdana, sans-serif" },
            { name: "Georgia", value: "Georgia, serif" }, { name: "Times New Roman", value: "Times New Roman, serif" },
            { name: "Courier New", value: "Courier New, monospace" }, { name: "Impact", value: "Impact, sans-serif" },
            { name: "Comic Sans MS", value: "Comic Sans MS, cursive" },
            { name: "Arial Black", value: "'Arial Black', Gadget, sans-serif" }, { name: "Tahoma", value: "Tahoma, Geneva, sans-serif" },
            { name: "Trebuchet MS", value: "'Trebuchet MS', Helvetica, sans-serif" }, { name: "Lucida Sans Unicode", value: "'Lucida Sans Unicode', 'Lucida Grande', sans-serif" },
            ...
            { name: "Brush Script MT", value: "'Brush Script MT', cursive" }, { name: "Lucida Console", value: "'Lucida Console', Monaco, monospace" },
            // { name: "Copperplate", value: "Copperplate, 'Copperplate Gothic Light', fantasy" }, // Often not available
            // { name: "Papyrus", value: "Papyrus, fantasy" } // Often not available
        ];
        ...
        const config = {
            ...
            // Player Name Styling
            playerNameFontFamily: availableFontFamilies[0].value, 
            playerNameFontSizeFactor: 0.06, 
            playerNameLetterSpacing: 0,
            playerNameTextColor: '#111111',
            playerNameTextOffsetY: 0,
            // Position Styling
            playerPositionFontFamily: availableFontFamilies[0].value,
            playerPositionFontSizeFactor: 0.04,
            playerPositionLetterSpacing: 0,
            playerPositionTextColor: '#333333',
            playerPositionTextOffsetY: 0,
        ...
        function populateFontDropdown(selectElementId) {
        ...
        function setupControls() {
            ...
            populateFontDropdown('nameFontFamilySelect');
            populateFontDropdown('posFontFamilySelect');
        ...
        function deletePreset(index) { if(index<0||index>=savedPresets.length)return;savedPresets.splice(index,1);localStorage.setItem(PRESETS_STORAGE_KEY,JSON.stringify(savedPresets));renderPresetsList();}
        function handleClearAllPresets() { if(confirm("Delete ALL presets?")){savedPresets=[];localStorage.removeItem(PRESETS_STORAGE_KEY);renderPresetsList();}}
        ...
        function _drawContentToCanvas(ctx, imageToDraw) {
            ...
            ctx.font = `normal ${posFontSize}px ${config.playerPositionFontFamily}`; // Use 'normal' weight for position
            ...
            ctx.letterSpacing = '0px'; // Reset global letterSpacing
        }
        ...
        function createCard(){ 
            ...
            const tempCanvas = document.createElement('canvas'); tempCanvas.width = textureCanvasWidth; tempCanvas.height = textureCanvasHeight; const tempCtx = tempCanvas.getContext('2d'); tempCtx.fillStyle = config.cardFrontBgColor; tempCtx.fillRect(0,0,tempCanvas.width, tempCanvas.height); tempCtx.fillStyle = '#555'; tempCtx.textAlign='center';tempCtx.textBaseline='middle';tempCtx.font='20px Arial'; tempCtx.fillText('Initializing Card...', tempCanvas.width/2, tempCanvas.height/2); const initialFrontTexture = new THREE.CanvasTexture(tempCanvas);
            ...
            targetReturnRotation.copy(cardMesh.rotation); scene.add(cardMesh);
            isInitialTextureReady = true; // Set flag *after* mesh is created
            updateCardImageAndRedraw(initialImageURL); // Load actual image
        }
        ...
        function handleRandomizeCard(){ 
            ...
            config.playerNameFontSizeFactor = 0.04 + Math.random() * 0.05; // Range 0.04 to 0.09
            config.playerNameLetterSpacing = Math.floor(Math.random() * 6 - 2); // Range -2 to 3
            config.playerNameTextColor = `rgb(${Math.floor(Math.random()*128)}, ${Math.floor(Math.random()*128)}, ${Math.floor(Math.random()*128)})`;
            config.playerNameTextOffsetY = Math.floor(Math.random() * 60 - 30); // Range -30 to 30

            config.playerPositionFontFamily = availableFontFamilies[Math.floor(Math.random() * availableFontFamilies.length)].value;
            config.playerPositionFontSizeFactor = 0.03 + Math.random() * 0.03; // Range 0.03 to 0.06
            config.playerPositionLetterSpacing = Math.floor(Math.random() * 4 - 1); // Range -1 to 2
