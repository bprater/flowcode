...
<head>
    ...
    <title>Fireworks Simulator - Particle Scale</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div id="controlsPanel">
            ...
            <div class="control-group">
             ...
             <div class="control-group">
                <label for="particleSizeScale">Particle Size Scale: <span id="particleSizeScaleValue">1.0</span>x</label>
                <input type="range" id="particleSizeScale" min="0.2" max="3.0" value="1.0" step="0.1">
            </div>
            <div class="control-group">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const particleSizeScaleSlider = document.getElementById('particleSizeScale'); // New Slider
            const particleSizeScaleValueSpan = document.getElementById('particleSizeScaleValue'); // New Span
            ...
            particleSizeScaleSlider.oninput = () => { // Event listener for the new slider
                particleSizeScale = parseFloat(particleSizeScaleSlider.value);
                particleSizeScaleValueSpan.textContent = particleSizeScale.toFixed(1);
            };
            particleSizeScaleValueSpan.textContent = parseFloat(particleSizeScaleSlider.value).toFixed(1); // Initialize span
            
            function hslToHex(h, s, l) { /* ... (same as before) ... */ l /= 100; const a = s * Math.min(l, 1 - l) / 100; const f = n => { const k = (n + h / 30) % 12; const color = l - a * Math.max(Math.min(k - 3, 9 - k, 1), -1); return Math.round(255 * color).toString(16).padStart(2, '0'); }; return `#${f(0)}${f(8)}${f(4)}`; }
            function hexToHsl(hex) { /* ... (same as before) ... */ let r = 0, g = 0, b = 0; if (hex.length == 4) { r = "0x" + hex[1] + hex[1]; g = "0x" + hex[2] + hex[2]; b = "0x" + hex[3] + hex[3]; } else if (hex.length == 7) { r = "0x" + hex[1] + hex[2]; g = "0x" + hex[3] + hex[4]; b = "0x" + hex[5] + hex[6]; } r /= 255; g /= 255; b /= 255; let cmin = Math.min(r,g,b), cmax = Math.max(r,g,b), delta = cmax - cmin, h = 0, s = 0, l = 0; if (delta == 0) h = 0; else if (cmax == r) h = ((g - b) / delta) % 6; else if (cmax == g) h = (b - r) / delta + 2; else h = (r - g) / delta + 4; h = Math.round(h * 60); if (h < 0) h += 360; l = (cmax + cmin) / 2; s = delta == 0 ? 0 : delta / (1 - Math.abs(2 * l - 1)); s = +(s * 100).toFixed(1); l = +(l * 100).toFixed(1); return [h, s, l]; }
            ...
            class Particle {
                ...
                draw() {
                    ...
                    if (this.effect === 'strobe' && this.lifetime > 0) { this.strobeCounter++; if (this.strobeCounter % 7 < 3) ctx.fillStyle = 'rgba(255,255,255,'+ (0.7 + Math.random()*0.3) +')'; }
                    
                    // Apply particle size scale here
                    const finalSize = Math.max(1, this.baseSize * particleSizeScale); // Ensure at least 1px
            ...
            class Firework { // ... (Firework constructor, update, explode, draw same as before)
